...
document.addEventListener("DOMContentLoaded", handleHideShow);
...
function handleHideShow() {
  ...
  for (let inlineModel of inlineModels) {
    ...
    for (let node of nodes) {
      let onChange;
      let hideFields = new Set(
        csvParse(node.getAttribute("--hideshow-fields"))
      );
    ...
    function fieldNameToFormRow(fieldName) {
      ...
      let formRow = inlineModel.getElementsByClassName(cls)[0];
      return formRow;
    }
